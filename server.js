...
app.get("/v1/auth/discord/callback", async (req, res) => {
  ...
  if (error) return res.redirect(`${frontendUrl}/pages/register.html?error=discord_denied`);
  if (!code) return res.redirect(`${frontendUrl}/pages/register.html?error=no_code`);

  try {
    // Exchange code for token using Node's built-in https (works on all Node versions)
    ...
    if (!userRes.ok) return res.redirect(`${frontendUrl}/pages/register.html?error=user_fetch_failed`);
    ...
    // Create token and redirect with it so frontend can store it
    const token = createToken(user.id);
...
app.post("/v1/security/honeypot", (req, res) => {
  ...
  const event = {
    ...
    message:   req.body.type === "honeypot_file_access"
                 ? `File access trap triggered: ${req.body.url || "unknown"}`
  ...
  console.log(`🍯 HONEYPOT: ${event.type} | IP: ${event.ip} | URL: ${event.url}`);
  res.json({ ok: true });
});
