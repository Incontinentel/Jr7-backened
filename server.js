...
const DB = {
  ...
  sessions: new Map(),
  securityEvents: [],
  news: [], // News articles managed via admin panel
  tournaments: [] // Tournaments managed via admin panel
};
  ...
  try {
    ...
    sessions.rows.forEach(row => {
      ...
      if (now - createdAt <= SESSION_MAX_AGE_MS) {
        SESSION_STORE.set(row.token, { userId: row.user_id, createdAt });
        restoredSessions++;
      }
    });
    console.log(`✅ Loaded ${users.rowCount} users, ${clans.rowCount} clans, ${matches.rowCount} matches, ${restoredSessions} active sessions from PostgreSQL`);
  } catch(err) {
...
async function createToken(userId) {
  ...
  SESSION_STORE.set(token, { userId, createdAt });
  if (pgPool) {
    try {
      await pgPool.query('INSERT INTO sessions (token,user_id,created_at) VALUES ($1,$2,$3) ON CONFLICT (token) DO NOTHING', [token, userId, createdAt]);
    } catch(err) { console.error("createToken persist error:", err.message); }
  }
  return token;
}
...
function getTokenUser(req) {
  ...
  // Expire after SESSION_MAX_AGE_MS
...
// ============================================
// DISCORD OAUTH2 — START LOGIN + CALLBACK
// ============================================
const DISCORD_REDIRECT_URI = process.env.DISCORD_REDIRECT_URI || "https://jr7-backened-production.up.railway.app/v1/auth/discord/callback";

// Frontend should send users here to begin Discord login (e.g. <a href="/v1/auth/discord">).
...
app.get("/v1/auth/discord/callback", async (req, res) => {
  ...
  if (!process.env.DISCORD_CLIENT_ID || !process.env.DISCORD_CLIENT_SECRET) {
    console.error("DISCORD_CLIENT_ID / DISCORD_CLIENT_SECRET is not set in environment variables!");
    return res.redirect(`${frontendUrl}/pages/register.html?error=server_misconfigured`);
  }
...
// USER ROUTES
// ============================================
app.post("/v1/users/:id/update", requireAuth, async (req, res) => {
...
// NOTE: this route must stay ABOVE "/v1/users/:id" — otherwise Express
// matches ":id" first and "leaderboard" gets treated as a user id (404).
...
app.post("/v1/clans/create", requireAuth, checkRateLimit, async (req, res) => {
  ...
  const logoUrl = (req.body.logo || '').startsWith('https://') ? req.body.logo : '';
  const clan = { id, name, tag, description, captain: req.user.username, captainId: req.user.id, primaryMode, recruiting, rank: DB.clans.size + 1, wins: 0, losses: 0, winRate: 0, streak: 0, memberCount: 1, color, textColor, borderColor: color + "40", logo: logoUrl, banner: '', createdAt: new Date().toISOString(), modes: primaryMode === "all" ? ["hp","sd","ctl"] : [primaryMode] };
...
app.post("/v1/clans/:id/update", requireAuth, async (req, res) => {
  const clan = DB.clans.get(req.params.id);
  if (!clan) return res.status(404).json({ message: "Clan not found" });
  ...
  if (name !== undefined) {
    const newName = sanitize(name, 40);
    if (!newName || newName.length < 2) return res.status(400).json({ message: "Clan name must be at least 2 characters" });
    clan.name = newName;
  }
  if (description !== undefined) clan.description = sanitize(description, 200);
  if (recruiting  !== undefined) clan.recruiting  = !!recruiting;
  if (logo   !== undefined) clan.logo   = logo.startsWith('https://')   ? logo   : clan.logo;
  if (banner !== undefined) clan.banner = banner.startsWith('https://') ? banner : clan.banner;
  if (color     !== undefined && /^#[0-9A-Fa-f]{6}$/.test(color))     { clan.color = color; clan.borderColor = color + "40"; }
  if (textColor !== undefined && /^#[0-9A-Fa-f]{6}$/.test(textColor)) clan.textColor = textColor;
  await saveClan(clan);
  res.json(clan);
});
...
app.post("/v1/matches/challenge", requireAuth, checkRateLimit, async (req, res) => {
  ...
  if (challenger.clanId === challengedClanId) return res.status(400).json({ message: "You cannot challenge your own clan" });
...
app.post("/v1/matches/:id/chat", requireAuth, (req, res) => {
  ...
  const type    = req.body.type === "system" ? "system" : "text";
  if (!message) return res.status(400).json({ message: "Message cannot be empty" });
  if (pusher) {
    pusher.trigger(`private-match-${req.params.id}`, "message", {
      message, type, username: req.user.username,
...
app.post("/v1/news", requireAdmin, (req, res) => {
  const { title, category, excerpt, content, icon, readTime, author, featured } = req.body;
  if (!title || !excerpt) return res.status(400).json({ message: "Title and content required" });
  const article = {
    id:       DB.news.length + 1,
    title:    sanitize(title, 120),
    category: category || "announcements",
    excerpt:  sanitize(excerpt, 300),
    content:  sanitize(content || excerpt, 5000),
    icon:     icon || "📢",
    readTime: readTime || "2 min",
    author:   sanitize(author || "Jr7 Staff", 50),
    date:     new Date().toISOString().slice(0, 10),
    featured: !!featured
  };
  DB.news.unshift(article); // newest first
  res.json(article);
});
...
app.post("/v1/admin/users/:id/edit", requireAdmin, async (req, res) => {
  ...
  if (username !== undefined) {
    const newName = sanitize(username, 24);
    if (!isValidUsername(newName)) return res.status(400).json({ message: "Invalid username format" });
    const taken = Array.from(DB.users.values()).find(u => u.id !== user.id && u.username.toLowerCase() === newName.toLowerCase());
    if (taken) return res.status(409).json({ message: "Username already taken" });
    user.username = newName;
  ...
  if (email !== undefined) {
    ...
    if (!isValidEmail(newEmail)) return res.status(400).json({ message: "Invalid email address" });
    const taken = Array.from(DB.users.values()).find(u => u.id !== user.id && u.email === newEmail);
